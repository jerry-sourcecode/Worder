...
function toFullSrc(filename: string): string {
  if (API.getEnvMode() == "release")
    return new URL(
      `file://${(API.getProjectRoot() + "/dist" + filename).replace(/\\/g, "/")}`,
    ).href;
  return filename;
}
 ...
 * 说明：
 *  - 更大的值表示记得越牢。
 *  - 不强烈惩罚历史错误次数；重点关注 lastWrong 是否在 lastCorrect 之后（即错误尚未被纠正）以及该错误距今是否很近。
 */
function calculateProficiency(
  ...
  let base =
    1 + baseCorrectWeight * correctCount - totalWrongWeight * wrongCount;
  if (base < 0.05) base = 0.05; // 保底，避免被完全压为0

  // --- 基准时间：优先使用最后一次正确复习的时间，否则使用第一次学习时间 ---
  const referenceTime =
    lastCorrect && !isNaN(lastCorrect.getTime()) ? lastCorrect : firstLearned;
  const deltaRefDays = Math.max(
    0,
    (now.getTime() - referenceTime.getTime()) / msPerDay,
  );
  ...
  // --- 考虑最近一次错误 ---
  if (lastWrong && !isNaN(lastWrong.getTime())) {
    const deltaWrongDays = Math.max(
      0,
      (now.getTime() - lastWrong.getTime()) / msPerDay,
    );

    // 情况 A：错误尚未被后续正确复习（lastWrong 在 lastCorrect 之后，或根本没有 lastCorrect）
    const wrongAfterLastCorrect =
      !lastCorrect || lastWrong.getTime() > lastCorrect.getTime();
    ...
    if (wrongAfterLastCorrect) {
      ...
      // 情况 B：错误已被后续正确纠正（lastCorrect 在 lastWrong 之后）
      // 这意味着用户纠正了错误。对熟练度给以“修复性”加成（与纠正的时间相关）
      const deltaSinceCorrectionDays = Math.max(
        0,
        (now.getTime() - lastCorrect!.getTime()) / msPerDay,
      );
...
function calcTimeDiffLevel(
  ...
  const diffDays = Math.floor(
    (nowDay.getTime() - calcDay.getTime()) / msPerDay,
  );
...
export { toFullSrc, calculateProficiency, calcTimeDiffLevel };
